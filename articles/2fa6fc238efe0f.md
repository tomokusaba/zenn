...
ここで大事なのは、**どのリポジトリで何を作るのか**、**記事の内容をどの方向で書くのか**、そして **どこまで進めてほしいのか** を最初に入れることです。今回でいえば、「GitHub Copilot app の記事を追加する」だけでなく、「チャットから PR 作成・merge までの流れを具体的に書く」と明示しておくと、session 全体のぶれが減ります。

逆に、ここが曖昧だと「記事本文だけ作る」「PR のところで止まる」といったずれが起きやすいです。**最初のチャットが、そのまま session 全体の設計図になる** と考えるとよいと思います。📝

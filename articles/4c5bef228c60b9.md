---
title: "スタックチャンに向けた Microsoft Foundry 音声アシスタントの技術検証 — リレーサーバーで途切れ問題を解決する"
emoji: "🤖"
...
スタックチャン（Stack-chan）は、M5Stack をベースにしたオープンソースの AI デスクトップロボットです。Kickstarter でのプロジェクトをきっかけに大きな注目を集めており、さまざまな音声アシスタント AI と組み合わせて使用できる点が大きな特徴です🤖

https://www.kickstarter.com/projects/m5stack/stackchan-the-first-co-created-open-source-ai-desktop-robot

スタックチャンでは OpenAI API や Google Gemini など様々な音声 AI バックエンドが利用可能ですが、今回は Microsoft Foundry（旧 Azure AI Foundry）が提供する GPT Realtime API をバックエンドとして使いたいという要求から、M5Stack CoreS3（ESP32-S3）上での動作について技術検証を行いました🔧

GPT Realtime API は音声入力をリアルタイムに受け取り、音声で応答を返す「speech in, speech out」型の対話を低レイテンシで実現できる API です。WebSocket 経由で 24kHz の PCM 音声をストリーミングし、GPT-4o ファミリーのモデルがリアルタイムに応答を生成します🎙️
...
リレーサーバー (PC / .NET 10)
    ...
    ▼
Microsoft Foundry — GPT Realtime API (gpt-realtime-mini)
```
...
この原因は、送信側でも同様の処理負荷が発生していたためと考えられます。マイクから取得した PCM 音声を Base64 にエンコードし、JSON に包んで TLS 暗号化して送信するという一連の処理に時間がかかり、音声チャンクの送信間隔が不均一になっていました。その結果、Azure 側が受け取る音声データにギャップが生じ、GPT-4o モデルによるネイティブの音声認識精度が著しく低下していたのです。なお、Realtime API ではモデルが音声を直接処理するため、音声入力の品質がそのまま認識精度に反映されます🎤
...
音声認識の精度も劇的に改善されました。音声チャンクが途切れなく一定間隔で Azure に到達するようになったため、GPT-4o モデルによるネイティブの音声処理が正確に機能するようになっています🎉
...
- [Audio events reference（Realtime API イベントリファレンス）— Microsoft Learn](https://learn.microsoft.com/azure/ai-foundry/openai/realtime-audio-reference?view=foundry-classic)
- [M5Unified ライブラリ — GitHub](https://github.com/m5stack/M5Unified)
- [M5Stack CoreS3 製品ページ](https://docs.m5stack.com/en/core/CoreS3)
- [Microsoft Foundry とは — Microsoft Learn](https://learn.microsoft.com/ja-jp/azure/ai-foundry/what-is-foundry?view=foundry-classic)

...
私はこれまで GitHub Copilot CLI をよく使ってきました。実際、ターミナルからそのまま流れをつなげたい場面では、今でも CLI がしっくりくると感じています。
...
そのうえで、GitHub Copilot app を触ってみると、**コードを書く前の迷い** や **複数の作業をさばくときの摩擦** が少し減るように感じました。少なくとも、これまで使ってきた範囲では、日常的に開く道具として GitHub Copilot app を選ぶ場面が増えています。

上の記事では、GitHub Copilot app の technical preview が **ワークフロー全体をどう変えそうか** という観点で整理しました。今回の記事ではもう少し手前に寄せて、**これまで使ってきた中で日常的に感じた使いやすさ** を紹介します。
...
:::message
本記事は「CLI より app が常に上」という話ではありません。どちらも使います。そのうえで、これまで使ってきた範囲で、日々の認知負荷や作業の入口という観点から app のほうが扱いやすいと感じたポイントを整理します。
:::
...
GitHub Docs では、セッションは **repository ごとに grouped** され、各セッションは独立した workspace として動くと説明されています。実際に触ると、**「今どの作業空間にいるか」** が視覚的に把握しやすいです。
...
| 🧠 認知負荷 | 「こっちのターミナルは何用だっけ？」が起きやすい | 迷いが減りやすい |

特に、Issue を見て、別のリポジトリの PR も気にしつつ、手元ではもう 1 本別件を進めるような日には、この差が効きます。

私は CLI の「ターミナルから全部進められる感じ」が好きです。ただ、**同時並行の整理** という意味では、app のほうがデフォルトで構造化されているぶん、私には扱いやすく感じられました。🗂️
...
一方、GitHub Copilot app の getting started では、onboarding 時や、後からリポジトリを接続して新しい session を作るときは **repository を選んでから始める** 流れになっています。私にとっては、これが意外と大きな違いでした。
...
この違いは小さく見えて、実際には **作業開始までの迷い** を減らしてくれます。とくに、普段から登録しているリポジトリ群の中で仕事を切り替えるような使い方では、相性がよさそうだと感じました。🚀

## 3. 定型的な作業は workflow にして呼び出せる

これも便利だと感じた点です。

GitHub Docs の scheduled workflows では、**recurring agent tasks を保存し、スケジュール実行または手動実行できる** と説明されています。私はここに価値を感じています。
...
こういうとき、毎回プロンプトを打ち直すより、workflow として保存しておけると楽です。たとえば次のような依頼です。
...
エージェント系のツールでは、最初の依頼の質が大事です。だからこそ、**入力そのものに余計な神経を使わなくてよい** というのは、見た目以上に大きいと感じています。⌨️

## 常用の道具として app を開くことが増えた

ここまで app のよかった点を中心に書いてきましたが、CLI を使わなくなったわけではありません。

ただ、今の私の感覚では、**まず最初に開く道具** として GitHub Copilot app を選ぶことが増えています。ざっくり整理すると、次のような使い分けです。
...
つまり、私にとって GitHub Copilot app は **常用の入口として選びやすい道具** に見えるようになってきました。CLI の価値が消えたわけではありませんが、今は **普段の起点は app、CLI は必要な場面で使う道具** という見え方に少しずつ変わっています。

## おわりに

GitHub Copilot app を触ってみて、私が最初に「よい」と感じたのは、派手な機能よりもむしろ次の 4 点でした。
...
ですが、日々の作業ではこうした小さな摩擦が積み重なります。GitHub Copilot app は、その摩擦を減らして **作業に入るまで** と **複数作業をさばくとき** を整えやすいように感じました。

CLI が好きな人ほど、この差は分かりやすいかもしれません。もし普段 GitHub Copilot CLI を使っていて、複数コンソールの整理や定型プロンプトの扱い、起動場所の迷いに少しでも引っかかりがあるなら、GitHub Copilot app も試してみる価値はありそうです。

少なくとも、これまで使ってきた範囲では、常用の道具として GitHub Copilot app を開く場面が増えています。🧭
